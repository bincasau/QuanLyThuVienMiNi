...
public class Login extends JFrame {
	...
	public Login() {
		...
		try {
			ImageIcon avatarIcon = new ImageIcon(getClass().getResource("/Pictures/avatar.png"));
		...
		btn_DangNhap.addActionListener(e -> {
			...
			if (isTaiKhoanPlaceholderActive || txt_TaiKhoan.getText().trim().isEmpty()) {
				JOptionPane.showMessageDialog(Login.this, "Vui lòng nhập tên đăng nhập", "Lỗi đăng nhập",
			...
			if (isMatKhauPlaceholderActive || txt_MatKhau.getPassword().length == 0) {
				JOptionPane.showMessageDialog(Login.this, "Vui lòng nhập mật khẩu", "Lỗi đăng nhập",
						JOptionPane.ERROR_MESSAGE);
			...
			try {
				loginController.login(username, password, new LoginController.LoginCallBack() {
					...
					public void onError(String errorMessage) {
						...
						JOptionPane.showMessageDialog(Login.this, errorMessage, "Lỗi hệ thống",
								JOptionPane.ERROR_MESSAGE);
					}
				});
			} catch (Exception ex) {
				...
				ex.printStackTrace();
				JOptionPane.showMessageDialog(Login.this, "Đã xảy ra lỗi không mong muốn: " + ex.getMessage(),
